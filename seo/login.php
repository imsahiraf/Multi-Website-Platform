 ...
 if(isset($_POST['login']))
{
    ...
    $query = $con->query("SELECT * from user where user = '$user' AND password='$password' AND access='1'");
    $count= mysqli_num_rows($query);
    if($count == 1)
    {
       $row= mysqli_fetch_assoc($query);
       $_SESSION['seologgedin']  = $row['buid'];
       $_SESSION['susername'] = $row['user'];
