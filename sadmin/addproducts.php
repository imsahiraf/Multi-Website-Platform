                                                ...
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-sm-10">
                                                        <select name="menu" class="form-control form-control-round">
                                                            ...
                                                            <?php
                                                            $category_list_array = array();
                                                               $selectpck = "SELECT * FROM `subcategory` where cuid = $id";
                                                               $sql = mysqli_query($con, $selectpck);
                                                                    while ($row = mysqli_fetch_array($sql)) {
...
if(isset($_POST['add'])){
        ...
        if (!in_array(strtolower($imageFileType),$extallowed)){
            echo '<script>alert("Sorry,For jpg & png extension files are allowed");document.location="addproducts.php"</script>';
            $status = false;
            $uploadOk = 0;
        }
        ...
        $slug_check = "SELECT * FROM subcategory WHERE slug = '$slug' and cuid= '$cuid'";
        $res = mysqli_query($con, $slug_check);
        if(mysqli_num_rows($res) > 0){
            echo '<script>alert("Change Permanent Link It Already exists!");document.location="addproducts.php?id='.$cuid.'"</script>';
            exit();
        }
        if($con->query("insert into subcategory(`name`,`slug`,`keywords`,`description`,`cid`,`menu`,`image`,`content`,`cuid`)values('".$name."','".$slug."','".$keywords."','".$description."','".$cid."','".$menu."','".$filename."','".$content."','".$cuid."')")){
        echo '<script>alert("Product Added Successful!");document.location="products.php?id='.$cuid.'"</script>';
        } else {
            echo '<script>alert("Data Not Inserted");document.location="addproducts.php?id='.$cuid.'"</script>';
        }
}
?>
