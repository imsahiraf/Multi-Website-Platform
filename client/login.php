...
if(isset($_GET['id']) && !empty($_GET['id'])){
        $id = $_GET['id'];
        if($query = $con->query("SELECT * from user where buid = '$id'")){
          ...
          $count= mysqli_num_rows($query);
            ...
            echo '<script>alert("Unable To Redirect Client Panel");document.location="../sadmin/client.php"</script>'; 
        }
    }
?>
<!DOCTYPE html>
...
<?php
 if(isset($_POST['login'])){
   ...
   if($query = $con->query("SELECT * from user where user = '$user' AND password='$password' AND staff=1")){
    $count= mysqli_num_rows($query);
    if($count == 1)
    {
       $row= mysqli_fetch_assoc($query);
       $_SESSION['staffloggedin']  = $row['buid'];
       $_SESSION['stusername'] = $row['user'];
       $_SESSION['success']="Login Successfull!";
         echo '<script>document.location="index.php"</script>';
         exit();
    }
    elseif($query = $con->query("SELECT * from user where user = '$user' AND password='$password' AND access=0 AND staff=0")){  
     $count= mysqli_num_rows($query);
    if($count == 1)
    {
       $row= mysqli_fetch_assoc($query);
